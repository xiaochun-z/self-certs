...
// CA Creation State
const caCreationMode = ref<'generate' | 'import'>('generate')
const newCaName = ref('')
const newCaPassword = ref('')
const importCaCrt = ref('')
const importCaKey = ref('')
...
const importCa = async () => {
  if (!newCaName.value || !importCaCrt.value || !importCaKey.value) {
    caErrorMessage.value = 'Please provide Name, Certificate, and Private Key.'
    return
  }
  isCaLoading.value = true
  caErrorMessage.value = ''
  try {
    const res = await fetch('/api/cert/ca/import', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: newCaName.value, caCrt: importCaCrt.value, caKey: importCaKey.value })
    })
    const data = await res.json()
    if (data.code === 200) {
      await loadCas()
      ...
      caErrorMessage.value = data.message || 'Failed to import CA.'
    }
  } catch (e: any) {
      ...
      <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg ring-1 ring-gray-200 dark:ring-gray-700 transition-colors duration-200">
        ...
        <div v-if="isCreatingCa || cas.length === 0" class="space-y-4">
          ...
          <div class="flex space-x-4 mb-4 border-b border-gray-200 dark:border-gray-700">
            <button @click="caCreationMode = 'generate'; caErrorMessage = ''" :class="caCreationMode === 'generate' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'" class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm cursor-pointer bg-transparent">Generate New CA</button>
            <button @click="caCreationMode = 'import'; caErrorMessage = ''" :class="caCreationMode === 'import' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'" class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm cursor-pointer bg-transparent">Import Existing CA</button>
          </div>

          <div v-if="caCreationMode === 'generate'" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
          ...
          <div v-if="caCreationMode === 'import'" class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">CA Name</label>
              <input v-model="newCaName" type="text" placeholder="e.g. My Imported Root CA" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">CA Certificate (CRT)</label>
              <textarea v-model="importCaCrt" rows="4" placeholder="-----BEGIN CERTIFICATE-----..." class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border font-mono p-2"></textarea>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">CA Private Key (KEY)</label>
              <textarea v-model="importCaKey" rows="4" placeholder="-----BEGIN PRIVATE KEY-----..." class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border font-mono p-2"></textarea>
            </div>
          </div>

          <button v-if="caCreationMode === 'generate'" @click="createCa" :disabled="isCaLoading" class="mt-4 w-full sm:w-auto px-6 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 transition cursor-pointer">
            {{ isCaLoading ? 'Generating...' : 'Generate Root CA' }}
          </button>
          <button v-if="caCreationMode === 'import'" @click="importCa" :disabled="isCaLoading" class="mt-4 w-full sm:w-auto px-6 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 transition cursor-pointer">
            {{ isCaLoading ? 'Importing...' : 'Import Root CA' }}
          </button>
